...
export default function Markers({ result }: dataProps) {
  ...
  const onLoadMarkers = useCallback(async () => {
    ...
    result?.map((store: any) => {
      ...
      const markerPosition = new window.kakao.maps.LatLng(address.y, address.x);
      ...
      // 커스텀 오버레이가 표시될 위치입니다
      const position = new window.kakao.maps.LatLng(address.y, address.x);
      ...
      window.kakao.maps.event.addListener(marker, 'click', () => {
        setCurrentStore({
          ...store,
          kakaoAddress: JSON.parse(store?.kakaoAddress),
        });
      });
    });
  }, [map, setCurrentStore]);
